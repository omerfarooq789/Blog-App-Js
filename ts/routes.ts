...
const route = (event = window.event) => {
  ...
  if (target.id === "logo") {
    const home = document.querySelector("#home") as HTMLAnchorElement;
...
const handleLocation = async () => {
  ...
  const addActive = document.getElementById(route.id) as HTMLAnchorElement;
  if (addActive) {
  ...
  route.scripts.forEach(async (script: string) => {
